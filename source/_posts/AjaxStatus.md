...
permalink: 
---
　　这篇文章主要介绍了各类Http请求状态(status)及其含义。
　　需要的朋友可以过来参考下，希望对大家有所帮助。Web服务器响应浏览器或其他客户程序的请求时，其应答一般由以下几个部分组成：一个状态行，几个应答头，一个空行，内容文档。下面是一个最简单的应答   ：
...
禁止访问：IIS 定义了许多不同的 403 错误，它们指明更为具体的错误原因：
...
在数据库连接池已满的情况下返回503。服务器返回503时可以提供一个Retry-After头。这个错误代码为IIS 6.0 所专用。
